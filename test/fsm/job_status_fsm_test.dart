...
void main() {
  ...
  group('Job lifecycle dispatcher', () {
    ...
    test('accept job remains available without preparing a quote', () async {
      final job = await _insertJob(JobStatus.prospecting);
      final machine = await buildJobMachine(job);
      final actions = await nextFromFsm(machine: machine, job: job);
      final accept = actions.singleWhere(
        (next) => next.action.eventType == AcceptJob,
      );
      expect(accept.action.label, 'Accept job');
      expect((await accept.fire()).status, JobStatus.toBeScheduled);
      expect(
        (await DaoLifecycleTransition().getByJob(job.id)).single.event,
        'AcceptJob',
      );
    });
