...
class _PlasterProjectScreenState extends DeferredState<PlasterProjectScreen>
  ...
  Future<void> _setSelectedLineAttributeMaskOverride(int? value) async {
    final selectedLineIndex = _selectedLineIndex;
    if (_rooms.isEmpty || selectedLineIndex == null) {
      return;
    }
    final lines = List<PlasterRoomLine>.from(_currentRoom.lines);
  ...
  Widget _buildEditorAttributeSettings() {
    final selectedLineIndex = _selectedLineIndex;
    final selectedLine = selectedLineIndex == null
        ? null
  ...
  RoomEditorDocument _toEditorDocument(_RoomBundle bundle) =>
      RoomEditorDocument(
        bundle: _toEditorBundle(bundle),
        constraints: _toEditorConstraints(bundle),
      );
  ...
  Future<void> _handleEditorWorkspaceCommand(RoomEditorCommand command) async {
    switch (command.type) {
      case RoomEditorCommandType.splitLine:
