...
/// Used to handle the oauth redirect on completion.
...
class LocalServerRedirectHandler extends RedirectHandler {
  ...
  Future<void> start() async {
    ...
    _running = true;
    server = await HttpServer.bind(InternetAddress.loopbackIPv4, config.port);
    _log('listening on http://${server!.address.host}:${server!.port}');

    server!.listen((request) async {
      if (request.uri.path == '/${config.redirectPath}') {
